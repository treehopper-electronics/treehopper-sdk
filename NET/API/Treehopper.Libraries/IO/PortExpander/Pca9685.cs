...
namespace Treehopper.Libraries.IO.PortExpander
{
    ...
    /// </summary>
    ///         You'll notice this driver lives in Treehopper.Libraries.IO, and not
    ///         Treehopper.Libraries.Displays (as other LED drivers do). While the PCA9685 is advertised as an
    ///         LED controller, it has several unique features not found in most LED drivers, such as adjustable PWM frequency
    ///         (via #Frequency), configurable output drive modes (via #OutputDrive)
    ///         supporting push-pull and open-drain outputs, and invertible output logic allowing this driver to be used with
    ///         both common-anode and common-cathode LED arrangements. 
    ///         
    ///         The PCA9685 also lacks two important feature found in
    ///         most LED drivers: programmable constant-current outputs (usually via an external resistor), and built-in
    ///         high-current open-drain output drivers. As a result, this IC is better suited as a general-purpose PWM-capable
    ///         GPIO port expander than as an LED driver, both in terms of electronic design and API interfacing design. To
    ///         this end, this library provides a collection of <see cref="Pwm" />-capable <see cref="Pins" />), instead of a
    ///         collection of <see cref="Treehopper.Libraries.Displays.Led" />s, found in other LED drivers. You can obtain a
    ...
    public class Pca9685 : IFlushable
    {
        ...
        /// </summary>
        public Collection<Pin> Pins { get; } = new Collection<Pin>();
        ...
        {
            var dc = pin.DutyCycle;
            var onTicks = (int) Math.Round(dc * 4096);
            if (onTicks == 4096)
            {
            ...
            {
                var delayTicks = pin.pinNumber * 4096 / 16; // stagger the outputs to reduce current inrush
    ...
    public class Pin : Pwm
    {
        ...
        /// </summary>
        /// <remarks>
        ///     This property will read the <see cref="Pca9685.Frequency" /> of the driver and change the <see cref="DutyCycle" />
        ///     property of this pin to set the desired pulse width. The accuracy of this property is dependent on the oscillator
        ///     source; the PCA9685 internal oscillator is not nearly as accurate as an external crystal oscillator, so if you need
        ///     accurate pulse width control, consider using an external oscillator.
        /// </remarks>
