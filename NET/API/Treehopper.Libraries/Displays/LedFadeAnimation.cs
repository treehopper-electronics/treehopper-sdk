...
namespace Treehopper.Libraries.Displays
{
    ...
    /// <remarks>
    ///     <para>
    ///         To fade an LED, construct an instance of this class, then set <see cref="Led" /> to the LED you wish to fade.
    ///         Set the <see cref="Duration" /> of the fade, and then call <see cref="RunAsync(double, double)" /> to actually
    ///         run the fade. You can switch the fader to use a different LED at any time — there's no need to construct
    ///         multiple instances of this class if only one LED needs to be faded at any given time.
    ///     </para>
    ///     <para>
    ///         The fade is performed using a linear interpolation; since <see cref="Led.Brightness" /> is linearly perceptual,
    ///         this will create a linear change in brightness during the course of the fade.
    ///     </para>
    /// </remarks>
    public class LedFadeAnimation
    {
        ...
        public Task RunAsync(double from = 0, double to = 1)
        {
            return Task.Run(() =>
            {
                ...
                // amount to step
                var amount = (to - from) / steps;

                // actual delay
                var ms = Duration / steps;
