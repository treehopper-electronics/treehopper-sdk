...
{
    /// <summary>
    /// Manages a static Libusb-1.0 <see cref="LibUsbSessionHandle"/> and "handle_events" thread for simplified asynchronous IO.
    /// </summary>
    /// <remarks>
    /// <para>This class contains its own <see cref="LibUsbSessionHandle"/> that is initialized with one of the overloaded <see cref="LibUsbEventHandler.Init()">MonoUsbEventHandler.Init()</see> functions.</para>
    /// <para>This class contains a static thread that execute <see cref="LibUsbApi.HandleEventsTimeout"/>. See the <see cref="Start"/> and <see cref="Stop"/> methods.</para>
    /// </remarks>
    public static class LibUsbEventHandler
    {
        private static readonly ManualResetEvent mIsStoppedEvent = new ManualResetEvent(true);
        private static bool mRunning;
        private static LibUsbSessionHandle mSessionHandle;
        ...
        /// </summary>
        /// <remarks>
        /// Used for MonoLibUsb members that require the <see cref="LibUsbSessionHandle"/> parameter.
        /// </remarks>
        public static LibUsbSessionHandle SessionHandle
        {
        ...
        {
            LibUsbSessionHandle sessionHandle = oHandle as LibUsbSessionHandle;

            mIsStoppedEvent.Reset();

            while (mRunning)
                LibUsbApi.HandleEventsTimeout(sessionHandle, ref mWaitUnixNativeTimeval);
        ...
        /// <para>Uses the MonoLibUsb <see cref="UnixNativeTimeval.Default"/> polling interval for <see cref="LibUsbApi.HandleEventsTimeout"/>.</para>
        /// </remarks>
        ...
        /// <para>
        /// Using a seperate thread which executes <see cref="LibUsbApi.HandleEventsTimeout"/> can simplify asynchronous I/O
            ...
            if (!IsStopped && mRunning)
            {
                ...
                if (bWait)
                {
                    ...
                    if (!bSuccess)
                    {
                        ...
                        throw new UsbException(typeof(LibUsbEventHandler), "Critical timeout failure! MonoUsbApi.HandleEventsTimeout did not return within the allotted time.");
