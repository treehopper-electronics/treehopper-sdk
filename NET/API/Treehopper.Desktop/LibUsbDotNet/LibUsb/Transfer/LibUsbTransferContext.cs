...
namespace LibUsbDotNet.LibUsb.Internal
{
    internal class LibUsbTransferContext : UsbTransfer, IDisposable
    {
        ...
        public override ErrorCode Wait(out int transferredCount, bool cancel)
        {
            transferredCount = 0;
            int ret = 0;
            global::LibUsb.LibUsbError monoError;
            ErrorCode ec;
            ...
            switch (iWait)
            {
                case 0: // TransferCompleteEvent
                    ...
                    string s;
                    monoError = LibUsbApi.MonoLibUsbErrorFromTransferStatus(mTransfer.Status);
                    ec = LibUsbApi.ErrorCodeFromLibUsbError((int)monoError, out s);
                ...
                case 1: // TransferCancelEvent
                    ...
                    if (ret != 0 || !bTransferComplete)
                    {
						...
						UsbError.Error(ec, ret, string.Format("Wait:Unable to cancel transfer or the transfer did not return after it was cancelled. Cancelled:{0} TransferCompleted:{1}", (global::LibUsb.LibUsbError)ret, bTransferComplete), EndpointBase);
                        return ec;
                    }
