...
namespace LibUsb.Transfer
{
    ...
    public struct LibUsbTransfer 
    {
        ...
        /// </note>
        /// </remarks>
        /// <returns><see cref="LibUsbError.Success"/> if the cancel succeeds, otherwise one of the other <see cref="LibUsbError"/> codes.</returns>
        public LibUsbError Cancel()
        {
            if (IsInvalid) return LibUsbError.ErrorNoMem;
        ...
        public void FillBulk(LibUsbDeviceHandle devHandle,
                         byte endpoint,
                         IntPtr buffer,
                         int length,
                         Delegate callback,
        ...
        /// <param name="userData">user data to pass to callback function</param>
        /// <param name="timeout">timeout for the transfer in milliseconds</param>
        public void FillInterrupt(LibUsbDeviceHandle devHandle,
                 byte endpoint,
                 IntPtr buffer,
                 int length,
                 Delegate callback,
        ...
        /// </remarks>
        /// <returns>
        /// <see cref="LibUsbError.Success"/> if the submit succeeds, 
        /// otherwise one of the other <see cref="LibUsbError"/> codes.
        /// </returns>
        public LibUsbError Submit()
        {
            if (IsInvalid) return LibUsbError.ErrorNoMem;
            return (LibUsbError)LibUsbApi.SubmitTransfer(handle);
        }
