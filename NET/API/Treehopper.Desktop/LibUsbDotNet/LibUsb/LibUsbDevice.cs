...
namespace LibUsbDotNet.LibUsb
{
    ...
    public class LibUsbDevice : UsbDevice, IUsbDevice
    {
        ...
        /// <summary>
        /// Gets a <see cref="LibUsbDevice"/> list of Libusb-1.0 devices.  
        /// </summary>
        /// <remarks>
        ...
        /// </remarks>
        public static List<LibUsbDevice> MonoUsbDeviceList
        {
            get
            {
                lock (OLockDeviceList)
        ...
        public bool ResetDevice()
        {
            ...
            if ((ret = LibUsbApi.ResetDevice((LibUsbDeviceHandle) mUsbHandle)) != 0)
            {
        ...
        public override bool GetDescriptor(byte descriptorType, byte index, short langId, IntPtr buffer, int bufferLength, out int transferLength)
        {
            ...
            int ret = LibUsbApi.GetDescriptor((LibUsbDeviceHandle) mUsbHandle, descriptorType, index, buffer, (ushort) bufferLength);
        ...
        {
            int ret = LibUsbApi.SetConfiguration((LibUsbDeviceHandle) mUsbHandle, config);
            if (ret != 0)
            {
        ...
        public override bool GetConfiguration(out byte config)
        {
            config = 0;
            int iconfig = 0;
            int ret = LibUsbApi.GetConfiguration((LibUsbDeviceHandle) mUsbHandle, ref iconfig);
            if (ret != 0)
            {
        ...
        {
            int ret = LibUsbApi.ClaimInterface((LibUsbDeviceHandle) mUsbHandle, interfaceID);
            if (ret != 0)
            {
        ...
        {
            int ret = LibUsbApi.ReleaseInterface((LibUsbDeviceHandle) mUsbHandle, interfaceID);
            if (ret != 0)
            {
        ...
        {
            int ret = LibUsbApi.SetInterfaceAltSetting((LibUsbDeviceHandle) mUsbHandle, mClaimedInteface, alternateID);
            if (ret != 0)
            {
        ...
        /// <summary>
        /// Initializes the <see cref="UsbEventHandler.SessionHandle"/> with <see cref="UsbEventHandler.Init()"/> and starts the static handle events thread with <see cref="UsbEventHandler.Start"/>. 
        /// </summary>
        /// <remarks>
        /// This is done automatically when needed.
        /// <para>Usually there is no need to call this functions externally.</para> 
        /// </remarks>
        public static void Init() { LibUsbApi.InitAndStart(); }
    }
}