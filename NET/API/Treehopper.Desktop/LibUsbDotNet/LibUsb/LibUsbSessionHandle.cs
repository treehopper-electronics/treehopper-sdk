...
namespace LibUsb
{
    ...
    /// individual libusb users - for example <see cref="LibUsbApi.SetDebug"/> will not affect the other 
    /// user of the library, and <see cref="SafeHandle.Close"/> will not destroy resources that the 
    /// other user is still using.</para>
    /// <para>Sessions are created when a new <see cref="LibUsbSessionHandle"/> instance is created and destroyed through <see cref="SafeHandle.Close"/>.</para>
    ...
    /// </remarks>
    public class LibUsbSessionHandle:SafeContextHandle
    {
        ...
        /// <summary>
        /// If the session handle is <see cref="SafeContextHandle.IsInvalid"/>, gets the <see cref="LibUsbError"/> status code indicating the reason.
        /// </summary>
