/// Portions of this code are from WinUsb_cs
/// Author: Jan Axelson
/// 
/// ***********************************************************************
...
namespace Treehopper
{
    ...
    internal sealed partial class WinUsbCommunications
    {
        ...
        {
            internal List<Byte> BulkInPipes = new List<byte>();
            internal List<Byte> BulkOutPipes = new List<byte>();
            internal List<Byte> InterruptInPipes = new List<byte>();
            internal List<Byte> InterruptOutPipes = new List<byte>();
            internal List<Byte> IsochronousInPipes = new List<byte>();
            internal List<Byte> IsochronousOutPipes = new List<byte>();
        ...
        internal SafeFileHandle GetDeviceHandle(String devicePathName)
        {
            try
            {
                ...
                SafeFileHandle deviceHandle = NativeMethods.CreateFile
                    (devicePathName,
        ...
        internal Boolean Open(string path, UInt32 pipeTimeout)
        {
            winUsbHandle = new SafeWinUsbHandle();
            myDeviceInfo = new DeviceInfo();
            deviceHandle = this.GetDeviceHandle(path);
            bool result = InitializeDevice(deviceHandle, pipeTimeout);
            return result;
        }
        ...
        internal Boolean InitializeDevice(SafeFileHandle deviceHandle, UInt32 pipeTimeout)
        {
            try
            {
                ...
                lock (thisLock)
                {
                        ...
                        for (var interfaceSetting = 0; interfaceSetting <= 1; interfaceSetting++)
                        {
                            ...
                            if (success)
                            {
                                ...
                                for (var i = 0; i <= ifaceDescriptors[interfaceSetting].bNumEndpoints - 1; i++)
                                {
                                    ...
                                    byte pipeId = pipeInfo[interfaceSetting].PipeId;

                                    if (((pipeInfo[interfaceSetting].PipeType ==
                                          NativeMethods.USBD_PIPE_TYPE.UsbdPipeTypeBulk) &
                                         IsUsbEndpointDirectionIn(pipeInfo[interfaceSetting].PipeId)))
                                    {
                                        
                                        myDeviceInfo.BulkInPipes.Add(pipeId);
                                        ...
                                        SetPipePolicy(
                                             ...
                                             pipeTimeout);
                                               ...
                                               NativeMethods.USBD_PIPE_TYPE.UsbdPipeTypeBulk) &
                                              IsUsbEndpointDirectionOut(pipeInfo[interfaceSetting].PipeId)))
                                    {
                                        myDeviceInfo.BulkOutPipes.Add(pipeId);
                                        ...
                                        SetPipePolicy
                                            (
                                             ...
                                             IsUsbEndpointDirectionIn(pipeInfo[interfaceSetting].PipeId))
                                    {
                                        myDeviceInfo.InterruptInPipes.Add(pipeId);
                                        ...
                                        SetPipePolicy
                                            (
                                             ...
                                             IsUsbEndpointDirectionOut(pipeInfo[interfaceSetting].PipeId))
                                    {
                                        myDeviceInfo.InterruptOutPipes.Add(pipeId);
                                        ...
                                        SetPipePolicy
                                            (
                                             ...
                                             IsUsbEndpointDirectionIn(pipeInfo[interfaceSetting].PipeId))
                                    {
                                        myDeviceInfo.IsochronousInPipes.Add(pipeId);
                                        ...
                                        SetPipePolicy
                                            (
                                             ...
                                             IsUsbEndpointDirectionOut(pipeInfo[interfaceSetting].PipeId))
                                    {
                                        myDeviceInfo.IsochronousOutPipes.Add(pipeId);
                            ...
                            {
                                Debug.WriteLine("NativeMethods.WinUsb_QueryInterfaceSettings failed");
                            }
                    ...
                    {
                        Debug.WriteLine("NativeMethods.WinUsb_Initialize failed");
                    }
                    return success;
                }
            }
        ...
        ///  
        internal void ReceiveDataViaBulkTransfer(int endpointIndex, UInt32 bytesToRead,
                ...
                lock (thisLock)
                {
                    ...
                    if (!(winUsbHandle.IsInvalid))
                    {
                        success = NativeMethods.WinUsb_ReadPipe
                            (winUsbHandle,
                             myDeviceInfo.BulkInPipes[endpointIndex],
                             dataBuffer,
                             bytesToRead,
                             ref bytesRead,
                             IntPtr.Zero);
        ...
        ///  
        internal Boolean ReceiveDataViaInterruptTransfer(int endpointIndex, UInt32 bytesToRead,
            ...
            try
            {
                ...
                lock (thisLock)
                {
                    ...
                    if (!(winUsbHandle.IsInvalid))
                    {
                        success = NativeMethods.WinUsb_ReadPipe
                            (winUsbHandle,
                             myDeviceInfo.InterruptInPipes[endpointIndex],
                             dataBuffer,
                             bytesToRead,
                             ref bytesRead,
                             IntPtr.Zero);
        ...
        ///  <returns> true on success, false on failure </returns>
        ///  
        internal Boolean ReceiveDataViaIsochronousTransfer(int endpointIndex, UInt32 bytesToRead,
                ...
                lock (thisLock)
                {
                    if (!(winUsbHandle.IsInvalid))
                    {
                        ...
                        success = NativeMethods.WinUsb_RegisterIsochBuffer
                            (winUsbHandle,
                            myDeviceInfo.IsochronousInPipes[endpointIndex],
                            dataInBuffer,
        ...
        internal void SendDataViaBulkTransfer(int endpointIndex, UInt32 bytesToWrite,
                ...
                lock (thisLock)
                {
                    ...
                    if (!(winUsbHandle.IsInvalid))
                    {
                        success = NativeMethods.WinUsb_WritePipe
                            (winUsbHandle,
                             myDeviceInfo.BulkOutPipes[endpointIndex],
                             dataBuffer,
                             bytesToWrite,
                             ref bytesWritten,
                             IntPtr.Zero);
        ...
        internal Boolean SendDataViaInterruptTransfer(int endpointIndex, UInt32 bytesToWrite,
            ...
            try
            {
                ...
                lock (thisLock)
                {
                    ...
                    if (!(winUsbHandle.IsInvalid))
                    {
                        success = NativeMethods.WinUsb_WritePipe
                            (winUsbHandle,
                             myDeviceInfo.InterruptOutPipes[endpointIndex],
                             dataBuffer,
                             bytesToWrite,
                             ref bytesWritten,
                             IntPtr.Zero);
        ...
        internal Boolean SendDataViaIsochronousTransfer(int endpointIndex, UInt32 bytesToWrite,
            ...
            try
            {
                ...
                lock (thisLock)
                {
                    if (!(winUsbHandle.IsInvalid))
                    {
                        ...
                        success = NativeMethods.WinUsb_RegisterIsochBuffer
                            (winUsbHandle,
                            myDeviceInfo.IsochronousOutPipes[endpointIndex],
                            dataOutBuffer,
        ...
        private Boolean IsUsbEndpointDirectionIn(Int32 endpointAddress)
        {
            try
            {
        ...
        private Boolean IsUsbEndpointDirectionOut(Int32 addr)
        {
            try
            {
