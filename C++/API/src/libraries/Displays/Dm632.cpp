...
namespace Treehopper {
    namespace Libraries {
        namespace Displays {
            Dm632::Dm632(Spi &spi, SpiChipSelectPin *lat, double speed) : ChainableShiftRegisterOutput(spi, lat, 32,
                                                                                                       speed),
                                                                          LedDriver(16, true, true) {
            }

            Dm632::Dm632(ChainableShiftRegisterOutput &upstreamDevice) : ChainableShiftRegisterOutput(upstreamDevice,
                                                                                                      32),
                                                                         LedDriver(16, true, true) {
            }
            ...
            void Dm632::update(Led &led) {
                if (led.state()) {
                    uint16_t brightness = (uint16_t) round(
                            Utility::brightnessToCieLuminance(led.brightness() * _brightness) * 65535);
